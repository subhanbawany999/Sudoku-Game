...
public class SudokuSolver {
    ...
    public static boolean solve(int x, int y, int move, int board[][]) {
        ...
        if (next != null) {
            //Starting the moves.
            for (int i = 1; i < 10; i++) { //Generating probable numbers
                if (isMoveValid(next.x, next.y, i, board)) {
                    board[next.x][next.y] = i;
                    if (solve(next.x, next.y, i, board)) {//This step is doing recursive backtracking
                        return true;
                    }
    ...
    public static int[][] solve(int[][] board) {
        int garbageBoard[][] = new int[9][9]; //For unsolvable case
        garbageBoard[0][0] = -1;
        int i = 0, j = 0;
        for (int k = 1; k < 10; k++) {
